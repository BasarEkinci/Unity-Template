...
namespace Syntac.SettingsSystem.Audio
{
    ...
    /// </summary>
    /// <remarks>
    /// The AudioMixer asset must expose a parameter named exactly "BGMVolume". A missing parameter makes
    ...
    /// </remarks>
    public sealed class BGMVolumeSetting : SettingsBase<float>
    {
        private const string k_ExposedParam = "BGMVolume";
        private readonly AudioMixer m_Mixer;

        [Inject]
        public BGMVolumeSetting(AudioMixer mixer)
        {
            m_Mixer = mixer;
        }

        public override string Key => "bgm_volume";
