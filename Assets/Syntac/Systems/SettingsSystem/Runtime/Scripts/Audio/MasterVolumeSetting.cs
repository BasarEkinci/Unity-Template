...
namespace Syntac.SettingsSystem.Audio
{
    ...
    /// </summary>
    /// <remarks>
    /// The AudioMixer asset must expose a parameter named exactly "MasterVolume". A missing parameter makes
    ...
    /// </remarks>
    public sealed class MasterVolumeSetting : SettingsBase<float>
    {
        private const string k_ExposedParam = "MasterVolume";
        private readonly AudioMixer m_Mixer;

        [Inject]
        public MasterVolumeSetting(AudioMixer mixer)
        {
            m_Mixer = mixer;
        }

        public override string Key => "master_volume";
