...
namespace Syntac.SettingsSystem.Audio
{
    ...
    /// </summary>
    /// <remarks>
    /// The AudioMixer asset must expose a parameter named exactly "VoiceVolume". A missing parameter makes
    ...
    /// </remarks>
    public sealed class VoiceVolumeSetting : SettingsBase<float>
    {
        private const string k_ExposedParam = "VoiceVolume";
        private readonly AudioMixer m_Mixer;

        [Inject]
        public VoiceVolumeSetting(AudioMixer mixer)
        {
            m_Mixer = mixer;
        }

        public override string Key => "voice_volume";
